...
var PCanvas = function PCanvas({ children, minScale, maxScale, style }) {
  ...
  React.Children.forEach(children, (sec) => {
    if (!sec || sec.type !== PSection) return;
    ...
    React.Children.forEach(sec.props.children, (ab) => {
      if (!ab || ab.type !== PArtboard) return;
...
// ─────────────────────────────────────────────────────────────
var PSection = function PSection({ id, title, subtitle, children, gap = 48 }) {
  const ctx = React.useContext(DCCtx);
  const sid = id ?? title;
  const all = React.Children.toArray(children);
  const artboards = all.filter((c) => c && c.type === PArtboard);
  const rest = all.filter((c) => !(c && c.type === PArtboard));
...
var PArtboard = function PArtboard() { return null; };
...
var DCArtboardFrame = function DCArtboardFrame({ sectionId, artboard, label, order, onRename, onReorder, onFocus, onDelete }) {
...
Object.assign(window, { PCanvas, PSection, PArtboard, DesignCanvas: PCanvas, DCSection: PSection, DCArtboard: PArtboard, DCPostIt });
